    ...
    view! {
        <div class="tabs-layout">
            ...
            <div class="tabs-viewport" class:drawer-open=move || app_store.get().drawer_open>
                {move || app_store.get().active_tab.map(|tab| {
                    view! { <TabContent tab_type=tab /> }.into_any()
                }).unwrap_or(().into_any())}
            </div>
        </div>
    }
...
fn TabDrawer() -> impl IntoView {
    ...
    view! {
        <>
            <div class="tab-drawer" class:drawer-open=move || app_store.get().drawer_open>
...
fn TabDrawerItem(tab_type: TabType, title: &'static str) -> impl IntoView {
    ...
    let is_active = move || app_store.get().active_tab.as_ref() == Some(&tab_type_active);
    let tab_type_edit = tab_type.clone();
    let is_edit = move || app_store.get().is_tab_edit_mode(&tab_type_edit);
    ...
    let tab_type_pen = tab_type.clone();
    let on_pen = Callback::new(move |_| {
        let tt = tab_type_pen.clone();
        app_store.update(|s| { s.toggle_tab_edit_mode(&tt); });
    });
    let is_overview = tab_type == TabType::Overview;
    ...
    view! {
        <div class="tab-drawer-item" class:active=is_active>
            ...
            {if is_overview {
                view! {
                    <button
                        class="tab-drawer-pen"
                        class:active=is_edit
                        title="Toggle edit mode"
                        on:click=move |_| on_pen.run(())
                    >"✎"</button>
                }.into_any()
            } else { ().into_any() }}
        </div>
